...
class GamePole:
    ...
    def get_secret_pole(self, is_secret=True):
        """
        ...
        for ship in self._ships:
            if sum(ship._cells) == len(ship) * 2:  # если корабль полностью утонул
...
class SeaBattle:
    ...
    def _get_people_coord(self):
        ...
        try:
            ...
            assert len(hit) == 2
            assert hit[0] >= 0 and hit[0] < self.size
            assert hit[1] >= 0 and hit[1] < self.size
    ...
    def start_game(self):
        ...
        while not self.people_pole.is_all_ships_sank() and not self.computer_pole.is_all_ships_sank():
            ...
            while people_hits:
                ...
                # hit = self._get_people_coord()
                hit = self._get_comp_coord(self.computer_pole)

                # Проверяем нанесен ли удар по кораблю, если да - поздравляем игрока, изменяем параметры корабля, повторяем удар
