class ValidateString:
	...
	def validate(self, string):
		return all([isinstance(string, str), 
				    len(string) >= self.min_length,
				    len(string) <= self.max_length])
...
frm = RegisterForm("123", "2345", "sc_liblist.ru")
assert frm.get_fields() == ["123", "2345", "sc_liblist.ru"], "метод get_fields вернул неверные данные"
